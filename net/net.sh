...
deployBootstrapValidator() {
  ...
  tar)
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$PANOPTES_ROOT"/solana-release/bin/* "$ipAddress:~/.cargo/bin/"
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$PANOPTES_ROOT"/solana-release/version.yml "$ipAddress:~/"
    ;;
  local)
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$PANOPTES_ROOT"/farf/bin/* "$ipAddress:~/.cargo/bin/"
...
prepareDeploy() {
  ...
  tar)
    ...
    (
      ...
      (cd "$PANOPTES_ROOT"; tar jxv) < "$tarballFilename"
