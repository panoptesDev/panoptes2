---
title: Using Hardware Wallets on the Panoptes CLI
---
...
The Panoptes CLI has first class support for hardware wallets. Anywhere
...
The Panoptes CLI supports the following hardware wallets:

- [Ledger Nano S and Ledger Nano X](hardware-wallets/ledger.md)

## Specify a Keypair URL

Panoptes defines a keypair URL format to uniquely locate any Panoptes keypair on a
...
`DERVIATION_PATH` is used to navigate to Panoptes keys within your hardware wallet.
...
and that any derived keys are Panoptes keys (Coin type 501). The single quote
indicates a "hardened" derivation. Because Panoptes uses Ed25519 keypairs, all
