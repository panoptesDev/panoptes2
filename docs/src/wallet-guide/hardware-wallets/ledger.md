...
This page describes how to use a Ledger Nano S or Nano X to interact with Panoptes
using the command line tools.  To see other solutions to interact with Panoptes with
your Nano, [click here](../ledger-live.md#interact-with-the-solana-network).

## Before You Begin

- [Set up a Nano with the Panoptes App](../ledger-live.md)
- [Install the Panoptes command-line tools](../../cli/install-solana-cli-tools.md)

## Use Ledger Nano with Panoptes CLI

1. Ensure the Ledger Live application is closed
2. Plug your Nano into your computer's USB port
3. Enter your pin and start the Panoptes app on the Nano
...
to interact with the Panoptes CLI. The command returns your Ledger's unique
...
To view any address, use the `panoptes-keygen pubkey` command, as shown below,
...
Any of the addresses displayed by these commands are valid Panoptes wallet
...
accounts are treated the same in Panoptes. This will change when your account
...
unlocked with the PIN, Ledger Live is not running, and the Panoptes App is open
on the device, showing "Application is Ready".

The `panoptes transfer` command is used to specify to which address to send tokens,
...
finalized on the Panoptes network. You can view details of this or any other
...
When the URL is not fully qualified, the Panoptes CLI will prompt you with
...
URLs using the Panoptes CLI `resolve-signer` command. For example, try
