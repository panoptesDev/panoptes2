---
title: Add Panoptes to Your Exchange
---

This guide describes how to add Panoptes's native token PANO to your cryptocurrency
exchange.
...
- to have a trusted gateway to the Panoptes mainnet-beta cluster to get data and
...
Panoptes nodes demand relatively high computing power to handle our fast blocks
and high TPS.  For specific requirements, please see
[hardware recommendations](../running-validator/validator-reqs.md).

To run an api node:

1. [Install the Panoptes command-line tool suite](../cli/install-solana-cli-tools.md)
...
[Current parameters for Mainnet Beta](../clusters.md#example-panoptes-validator-command-line-2)
...
`--limit-ledger-size` if desired. Check `panoptes-validator --help` for the
...
ensure you miss as little data as possible. Running the panoptes software as a
systemd service is one great option.

For monitoring, we provide
[`panoptes-watchtower`](https://github.com/solana-labs/solana/blob/master/watchtower/README.md),
which can monitor your validator and detect with the `panoptes-validator` process
is unhealthy. It can directly be configured to alert you via Slack, Telegram,
Discord, or Twillio. For details, run `panoptes-watchtower --help`.
...
`panoptes-validator` command to receive historical ledger data instead of a
snapshot.
...
traffic from all other Panoptes validators.   While this is the most efficient mode of
operation, and is strongly recommended, it is possible to restrict the
validator to only require inbound traffic from one other Panoptes validator.
...
Panoptes accounts do not require any on-chain initialization; once they contain
some PANO, they exist. To set up a deposit account for your exchange, simply
generate a Panoptes keypair using any of our [wallet tools](../wallet-guide/cli.md).

We recommend using a unique deposit account for each of your users.

Panoptes accounts are charged [rent](developing/programming-model/accounts.md#rent) on creation and once per
...
JSON-RPC service of your Panoptes api node.
...
To accommodate a user's request to withdraw PANO, you must generate a Panoptes
transfer transaction, and send it to the api node to be forwarded to your
cluster.

### Synchronous

Sending a synchronous transfer to the Panoptes cluster allows you to easily ensure
that a transfer is successful and finalized by the cluster.

Panoptes's command-line tool offers a simple command, `panoptes transfer`, to
...
```bash
panoptes transfer <USER_ADDRESS> <AMOUNT> --keypair <KEYPAIR> --url http://localhost:8328
```

The [Panoptes Javascript SDK](https://github.com/solana-labs/solana-web3.js)
...
```bash
panoptes fees --url http://localhost:8328
```
...
```bash
panoptes transfer <USER_ADDRESS> <AMOUNT> --no-wait --blockhash <RECENT_BLOCKHASH> --keypair <KEYPAIR> --url http://localhost:8328
```
...
[`getFees` endpoint](developing/clients/jsonrpc-api.md#getfees) or `panoptes fees`, the
...
The address of a normal account in Panoptes is a Base58-encoded string of a
...
token creation and exchange on the Panoptes blockchain.
...
Be sure to test your complete workflow on Panoptes devnet and testnet
...
run `panoptes airdrop 10` to obtain some devnet or testnet PANO for developement and testing.
