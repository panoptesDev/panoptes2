...
A stake account on Panoptes can be used to delegate tokens to validators on
...
Stake accounts on Panoptes also work differently than those of other Proof-of-Stake
blockchain networks that you may be familiar with.  This document describes the
high-level structure and functions of a Panoptes stake account.
...
Like other types of accounts on the Panoptes network, a stake account that has a
...
Stake account details can be viewed on the Panoptes Explorer by copying and pasting
