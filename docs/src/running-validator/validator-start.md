...
## Configure Panoptes CLI

The panoptes cli includes `get` and `set` configuration commands to automatically
set the `--url` argument for cli commands. For example:

```bash
panoptes config set --url http://devnet.panoptes.org
```

While this section demonstrates how to connect to the Devnet cluster, the steps
are similar for the other [Panoptes Clusters](../clusters.md).
...
```bash
panoptes transaction-count
```

View the [metrics dashboard](https://metrics.panoptes.org:3000/d/monitor/cluster-telemetry) for more
...
the `--cuda` argument to `panoptes-validator`.
...
### Linux
#### Automatic
The panoptes repo includes a daemon to adjust system settings to optimize performance
(namely by increasing the OS UDP buffer and file mapping limits).

The daemon (`panoptes-sys-tuner`) is included in the panoptes binary release. Restart
...
You can generate a custom vanity keypair using panoptes-keygen. For instance:

```bash
panoptes-keygen grind --starts-with e1v1s
```
...
## More Panoptes CLI Configuration

Now that you have a keypair, set the panoptes configuration to use your validator
...
“vote-account-keypair.json” in your Panoptes runtime directory:
...
argument to `panoptes-validator`. You can specify multiple ones by repeating the argument `--trusted-validator <PUBKEY1> --trusted-validator <PUBKEY2>`.
...
panoptes-validator \
  ...
  --rpc-port 8328 \
  --entrypoint devnet.panoptes.org:10015 \
  --limit-ledger-size \
...
> `panoptes-validator --identity ASK ... --authorized-voter ASK ...` and you will be
...
example, `panoptes-validator --dynamic-port-range 11000-11010 ...` will restrict
...
if desired. Check `panoptes-validator --help` for the default limit value used by
...
Now create `/home/sol/bin/validator.sh` to include the desired `panoptes-validator`
...
The validator log file, as specified by `--log ~/panoptes-validator.log`, can get
...
`panoptes-validator` command-line.
...
Add the `--snapshot-compression none` argument to your `panoptes-validator`
...
Now add the `--accounts /mnt/solana-accounts` argument to your `panoptes-validator`
command-line arguments and restart the validator.
