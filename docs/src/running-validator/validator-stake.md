...
this step, you should see the “validator-stake-keypair.json” in your Panoptes
runtime directory.
...
- View your vote account:`panoptes vote-account ~/vote-account-keypair.json` This displays the current state of all the votes the validator has submitted to the network.
- View your stake account, the delegation preference and details of your stake:`panoptes stake-account ~/validator-stake-keypair.json`
- `panoptes validators` displays the current active stake of all validators, including yours
- `panoptes stake-history` shows the history of stake warming up and cooling down over recent epochs
...
- Once your stake is warmed up, you will see a stake balance listed for your validator by running `panoptes validators`

## Monitor Your Staked Validator

Confirm your validator becomes a [leader](../terminology.md#leader)

- After your validator is caught up, use the `panoptes balance` command to monitor the earnings as your validator is selected as leader and collects transaction fees
- Panoptes nodes offer a number of useful JSON-RPC methods to return information about the network and your validator's participation. Make a request by using curl \(or another http client of your choosing\), specifying the desired method in JSON-RPC-formatted data. For example:
...
- `getVoteAccounts` This will tell you how much active stake your validator currently has. A % of the validator's stake is activated on an epoch boundary. You can learn more about staking on Panoptes [here](../cluster/stake-delegation-and-rewards.md).
