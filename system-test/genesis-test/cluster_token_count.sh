...
usage: $0 [cluster_rpc_url]
 ...
 Required arguments:
   cluster_rpc_url  - RPC URL and port for a running Panoptes cluster (ex: http://34.83.146.144:8328)
EOF
  exit $exitcode
}
...
function get_token_capitalization {
  ...
  totalSupplySafe=$((totalSupplyLamports / LAMPORTS_PER_PANO))
...
function get_program_account_balance_totals {
  ...
  totalAccountBalancesSafe=$((totalAccountBalancesLamports / LAMPORTS_PER_PANO))
