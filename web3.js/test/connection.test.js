...
test('get program accounts', async () => {
  ...
  await connection.requestAirdrop(account0.publicKey, LAMPORTS_PER_PANO);
  await connection.requestAirdrop(account1.publicKey, 0.5 * LAMPORTS_PER_PANO);
...
test('stake activation should return activating for new accounts', async () => {
  ...
  await connection.requestAirdrop(authorized.publicKey, 2 * LAMPORTS_PER_PANO);
...
test('account change notification', async () => {
  ...
  await connection.requestAirdrop(owner.publicKey, LAMPORTS_PER_PANO);
  try {
    const transaction = new Transaction().add(
      SystemProgram.transfer({
